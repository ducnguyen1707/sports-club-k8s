-- Copied verbatim from app/Files/sports_club_db.sql
-- Upstream commit: f5b6ed342aafe35b9b6d37acb7424f73173ec3ce
-- Refresh with: cp app/Files/sports_club_db.sql k8s/base/mysql-init.sql
--
-- NOTE: dump contains no CREATE DATABASE/USE — the mysql image's
--       MYSQL_DATABASE env var creates it and runs this against it.
-- NOTE: countGender() is DEFINER=root@localhost. Init runs as root so
--       creation succeeds; the app user gets EXECUTE via GRANT ALL.
